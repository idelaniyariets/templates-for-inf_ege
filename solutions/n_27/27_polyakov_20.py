...
def get_cluster(p0):
    cluster = [p for p in data if dist(p, p0) < 1.9]
    if cluster:
        for p in cluster:
            data.remove(p)
...
px = abs(int((sum([p[0] for p in centroids]) / len(centroids))*10000))
py = abs(int((sum([p[1] for p in centroids]) / len(centroids))*10000))
print(px,py)